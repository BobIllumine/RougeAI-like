...
public class DefaultAttack : Action, IEffect, ITarget
{
    ...
    public Dictionary<PropertyInfo, object> GetModifiedStats(BaseState state)
    {
        ...
        foreach(string stat in statChange)
        {
            ...
            print($"{prop.Name} = {_statChangeMapping[stat](prop.GetValue(state))}, curhp_d={curHP_d}, {(int)prop.GetValue(state) + curHP_d}");
    ...
    public DefaultAttack Initialize(BaseAnimResolver animResolver, params (string, object)[] stat_kwargs) 
    {
